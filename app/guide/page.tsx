...
function Cell({ val }: { val: boolean | string }) {
  if (val === false) return <td style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '1.1rem' }}>—</td>;
  if (val === true) return <td style={{ textAlign: 'center', color: '#16A34A', fontSize: '1.1rem', fontWeight: 700 }}>✔</td>;
  return <td style={{ textAlign: 'center', fontSize: '0.78rem', fontWeight: 600, color: '#1B3464' }}>{val}</td>;
}
...
export default function GuidePage() {
  return (
    <main style={{ background: '#F4F6FA', minHeight: '100vh', fontFamily: "'Segoe UI', Arial, sans-serif", color: '#1A1A2E' }}>
      ...
      {/* Hero */}
      <div style={{ background: '#1B3464', color: 'white', padding: '48px 24px 40px', textAlign: 'center' }}>
        <img src="/assets/logo-gdigital.png" alt="G+Digital Success" style={{ height: 56, marginBottom: 20, objectFit: 'contain' }} />
        ...
        <Section id="section-2" num={2} title="Vos fonctionnalités selon votre plan">
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
              <thead>
                <tr>
                  <th style={{ background: '#1B3464', color: 'white', padding: '10px 14px', textAlign: 'left', borderRadius: '6px 0 0 0' }}>Fonctionnalité</th>
                  {['Starter', 'Pro', 'Business', 'Business Équipe'].map((p, i) => (
                    <th key={i} style={{ background: i === 1 ? '#D4A843' : '#1B3464', color: i === 1 ? '#1B3464' : 'white', padding: '10px 14px', textAlign: 'center', borderRadius: i === 3 ? '0 6px 0 0' : 0 }}>{p}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {PLANS.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#F4F6FA' }}>
                    <td style={{ padding: '8px 14px', fontWeight: 600, borderBottom: '1px solid #E8ECF4' }}>{row.feature}</td>
        ...
        <Section id="section-7" num={7} title="Support & assistance">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
            ...
            <div style={{ background: 'white', border: '1px solid #E8ECF4', borderRadius: 10, padding: '20px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.85rem', color: '#6B7280' }}>digitalsucces.tech/guide<br />Toujours à jour, accessible depuis votre téléphone.</div>
            </div>
          </div>
        </Section>
...
function Section({ id, num, title, children }: { id: string; num: number; title: string; children: React.ReactNode }) {
  return (
    <section id={id} style={{ marginBottom: 40, scrollMarginTop: 20 }}>
      <div style={{ background: '#1B3464', color: 'white', padding: '10px 18px', borderRadius: '8px 8px 0 0', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 0 }}>
